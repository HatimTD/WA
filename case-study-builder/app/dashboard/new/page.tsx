  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setSubsidiaryDefault({ currency: res.currency, subsidiaryName: res.subsidiaryName, source: res.source, userRole: res.userRole });
      ...
      <Card role="article" className="dark:bg-card dark:border-border">
        ...
        <CardContent className="space-y-6">
          ...
          {STEPS[currentStep - 1]?.title === 'Cost Reduction Analysis' && (
            <StepCostCalculator formData={formData} updateFormData={updateFormData} subsidiaryCurrency={subsidiaryDefault?.currency} subsidiaryName={subsidiaryDefault?.subsidiaryName} subsidiarySource={subsidiaryDefault?.source} userRole={subsidiaryDefault?.userRole} />
          )}
          {STEPS[currentStep - 1]?.title === 'Finalise' && (
            <StepFive formData={formData} updateFormData={updateFormData} highlightedFields={highlightedFields} subsidiaryCurrency={subsidiaryDefault?.currency} subsidiaryName={subsidiaryDefault?.subsidiaryName} subsidiarySource={subsidiaryDefault?.source} userRole={subsidiaryDefault?.userRole} />
          )}
        </CardContent>
      </Card>
